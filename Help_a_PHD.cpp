...
(1≤N≤1000) denoting the number of testcases. Then follow N lines with either “P=NP” or an addition problem on the form “a+b”, where a,b∈[0,1000]
...
int main()
{
    ...
    for(int i = 0; i < n; i++){
        ...
        if(in[0] == 'P'){
            ...
            one = stoi(in.substr(0,in.find("+")));
            two = stoi(in.substr(in.find("+")+1));
            cout << one + two<<"\n";
        }
    }
